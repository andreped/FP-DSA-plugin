...
def main(args):
    ...
    annot_fname = os.path.splitext(os.path.basename(args.outputEpitheliumAnnotationFile))[0]
    annotation = {
        "name": annot_fname + '-nuclei-' + args.nuclei_annotation_format,
        "elements": annot_list
    }

    with open(args.outputEpitheliumAnnotationFile, 'w') as annotation_file:
        json.dump(annotation, annotation_file, separators=(',', ':'), sort_keys=False)

    total_time_taken = time.time() - total_start_time

    file_exist = os.path.exists(args.outputEpitheliumAnnotationFile)
    print("\n>> Does JSON file exist:", file_exist)

    if file_exist:
        print("\n>> JSON file size on disk [MB]:", os.path.getsize(args.outputEpitheliumAnnotationFile) / (1024.0 ** 2))
